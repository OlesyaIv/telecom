...
scale = 100; % во сколько "раст€нуть" сигналы
% e.g. дл€ [1 0], scale=2: [1 1 0 0];
...
grid on;
xlabel('Time (secs)');
linkaxes(ax(1:2), 'xy'); % св€зываем оси по X и Y
...
% выводим график коррел€ции
ax(3) = subplot(313);
plot(lag/Fs, real(C), 'b');
ylabel('Amplitude');
grid on
title('Cross-correlation between Template and Signal');
axis([-1.05 1.05 -10 210]); % коррел€ци€ мб отрицательной по Y
...
if(SampleDiff == 0) SampleDiff = 1; end; % r(0:end) - нельз€
...
p = zeros(1, lm); % выделение пам€ти под пакет с данными
for i=0:lm-1
